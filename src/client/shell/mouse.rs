...
impl ClientShellState {
    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        outcome.repaint |= self.update_local_path_hover(mouse);
                ...
                .contains(crossterm::event::KeyModifiers::CONTROL)
        {
                ...
                .cloned()
            {
                if self.active_endpoint_id.is_local() {
                    if let Some((pane_id, raw)) = self.local_path_at_point(mouse.column, mouse.row)
                    {
                        if let Some(cwd) = self.snapshot.as_ref().and_then(|snapshot| {
                            snapshot
                                .panes
                                .iter()
