...
fn local_path_click_surface(text: &str) -> PaneSurfaceFrame {
    ...
    pane_surface.panes[0].rect = area;
    pane_surface.panes[0].inner_rect = area;
    pane_surface
}

fn local_path_click_event(state: &ClientShellState, control: bool) -> MouseEvent {
...
fn ctrl_local_path_click_resolves_against_foreground_cwd_then_pane_cwd() {
    for (foreground_cwd, expected_cwd) in [(Some("/repo/task"), "/repo/task"), (None, "/repo")] {
        ...
        let mut pane_snapshot = snapshot();
        pane_snapshot.panes[0].foreground_cwd = foreground_cwd.map(str::to_owned);
        state.set_snapshot(Box::new(pane_snapshot));
        state.set_pane_surface(local_path_click_surface("./output/report.csv:12"));
...
#[test]
fn ctrl_local_path_click_consumes_drag_and_release_without_pty_input() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    let mut pane_surface = local_path_click_surface("./output/report.csv");
...
#[test]
fn local_path_without_control_still_starts_normal_text_selection() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    state.set_pane_surface(local_path_click_surface("./output/report.csv"));
    state.compose(106, 20).expect("local path frame");
    let click = local_path_click_event(&state, false);
    let outcome = state.handle_raw_events(vec![RawInputEvent::Mouse(click)]);
...
#[test]
fn ctrl_local_path_without_pane_cwd_uses_endpoint_link_fallback() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    let mut pane_snapshot = snapshot();
    pane_snapshot.panes[0].foreground_cwd = None;
    pane_snapshot.panes[0].cwd = None;
    state.set_snapshot(Box::new(pane_snapshot));
    state.set_pane_surface(local_path_click_surface("./output/report.csv"));
    state.compose(106, 20).expect("local path frame");
    let click = local_path_click_event(&state, true);
...
#[test]
fn ctrl_remote_path_click_never_reveals_a_client_local_file() {
    ...
    let mut remote_surface = local_path_click_surface("./output/report.csv");
    remote_surface.boot_id = "remote-boot".into();
    state.set_pane_surface(remote_surface);
    state.compose(106, 20).expect("remote path frame");
    let click = local_path_click_event(&state, true);
    let outcome = state.handle_raw_events(vec![RawInputEvent::Mouse(click)]);
    assert!(matches!(
        &outcome.actions[..],
...
#[test]
fn ctrl_http_url_with_path_retains_endpoint_link_activation() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    state.set_pane_surface(local_path_click_surface(
        "https://example.test/output/report.csv",
    ));
    state.compose(106, 20).expect("URL frame");
    let mut click = local_path_click_event(&state, true);
    // Click the apparent file-path portion, not just the URL scheme.
    click.column = state.hits.panes[0].inner_rect.x + 26;
    let outcome = state.handle_raw_events(vec![RawInputEvent::Mouse(click)]);
    assert!(matches!(
        &outcome.actions[..],
        [ClientShellAction::Endpoint { request, .. }]
